...
public class AFNelsonSiegel extends IrModel {
	...
	// 2023.06.09 add 珥덇린紐⑥닔�뿉 sigma 異붽�
	...
	// 23.06.05 add for �궡遺�紐⑦삎 stoScen
            ...
            double ltfrL, double ltfrA, int ltfrT, double liqPrem, double term, double minLambda, double maxLambda, int nf, int prjYear, double accuracy, int itrMax, double confInterval, double epsilon, int randomGenType, int seedNum)
	{
		...
		this.randomNumberGaussian(3); // �궃�닔 �깮�꽦 (nf = 3)
	}
...
//		this.epsilon = mode.toUpperCase().equals("AFNS")? this.epsilon: Math.sqrt(residualSumOfSquares(this.lambda)/ elementCnt );
	...
	private double residualSumOfSquares(double lambda) {
		...
		for(int i=0; i<this.iRateHis.length; i++) {
			...
			totalSum = reg.calculateTotalSumOfSquares();
			rs = reg.calculateRSquared();
			
//			log.info("Y array  : {},{},{},{},{},{},{}", i,lambda, rslt[0],rslt[1],rslt[2], residue[i],  yArray);

			residualSum += residue[i];
			cnt  = cnt + 1;
//			log.info("residualSum, lambda1111111111111111 :{}, {},{},  {},{}", i, rslt);
		}
		
//		TODO
		if(cnt * tenor.length > 0) {
			this.epsilon = Math.sqrt(residualSum / ( cnt * tenor.length));
//			log.info("residualSum, lambda :{},{}, {},{},{},{},{},  {},{}",irCurveId, residualSum, lambda, cnt * tenor.length, this.epsilon, rs, totalSum);
		}
...
//			log.info("coeff Lt : {},{},{},{}", i, coeffLt[i]*100, coeffSt[i]*100, coeffCt[i]*100);
		}
...
//	        temSum = residualL[i]+residualS[i] + residualC[i] + temSum;
//	        log.info("Residual Calc :{},{},{},{}", i, residualL[i], residualS[i], residualC[i]);
	    }

//	    log.info("Residual Calc :{},{},{},{}", temSum);
	    // 
	    ...
	    // 	Error Matrix
	    SimpleMatrix omegaHat = new SimpleMatrix(resiE.transpose().mult(resiE).scale(1.0 / (coeffLt.length - 2))); 
...
//			fpScale[i] = 1000;
		...
		log.info("Kalman Filtering : {}, {}, {}", LocalDateTime.now(), paras);
		try {
...
//						                                      , new ObjectiveFunction(fp)
				...
				log.info("Kalman Filtering Iteration : {}, {}, {}, {}", i, result.getValue(), LocalDateTime.now(), result.getPoint());
...
//		SimpleMatrix H      = new SimpleMatrix(toDiagMatrix(Math.pow((paras[13] * 0.001), 2), this.tenor.length));   // R in Excel!!!

		SimpleMatrix B      = new SimpleMatrix(factorLoad(Lambda, this.tenor, true));               //H in Excel !!!
		...
		for(int i=0; i<this.iRateHis.length; i++) {
			...
			SimpleMatrix Yimp  = B.mult(Xhat).plus(C.scale(mode.toUpperCase().equals("AFNS") ? 1 : 0));		// TODO : Adjust Term ???
//			SimpleMatrix Yimp  = B.mult(Xhat).plus(C.scale(mode.toUpperCase().startsWith("AFNS") ? 1 : 0));		// TODO : Adjust Term ???
			...
			this.coeffLt[i] = PrevX.get(0,0);
			this.coeffSt[i] = PrevX.get(1,0);  
			this.coeffCt[i] = PrevX.get(2,0);
		}
		return -logLike;
	}
...
//		log.info("factorLoad : {}, {}, {}, {}, {}", lambda, fLoad[0][0], fLoad[0][1], fLoad[0][2]);
		return fLoad;
	}
	...
	// 230530 add initParam setting : IrParamAfnsCalc 
